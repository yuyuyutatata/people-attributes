...
(async function boot() {
  ...
  async function dbMaintenance(){
    ...
    }catch(e){ console.warn('DB maintenance skipped:', e?.message||e); }
  }
  await dbMaintenance();
  ...
  const addPersonVec = vec => {
    for(const p of people){
      for(const v of p.vecs){
        if (cosSim(vec, v) >= MEMORY_SIM_TH){ p.vecs.push(vec); if(p.vecs.length>3) p.vecs.shift(); return; }
      }
    }
  ...
  function assignDetectionsToTracks(dets){
    ...
    for(const p of pairs){
      ...
      // どれかが十分なら許容（厳しすぎる AND を緩める）
      const passSim  = p.sim >= SIM_MIN;
      const passIOU  = p.ov  >= IOU_MIN;
      const passDist = p.dist <= DIST_MAX_RATIO;
  ...
  async function resetAll(){
    ...
    try{
      ...
      try{ await deleteFacesDB(); }catch(e){ alert('DBの削除がブロックされました。他タブを閉じて再度お試しください。\n'+(e?.message||e)); }
      tracks.clear?.(); if (Array.isArray(people)) people.length=0; if (typeof nextTrackId!=='undefined') nextTrackId=1;
    ...
    }catch(err){ console.error(err); alert('全リセット中にエラー：'+(err?.message||err)); }
  }
