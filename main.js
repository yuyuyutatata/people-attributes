...
(async function boot() {
  ...
  const logEl    = document.getElementById('log'); // ← 表示は使わない（非表示にする）
  ...
  let uniqSet = loadUniqSet(); // “その日すでに数えたID”

  // ===== 類似判定（ブレ吸収） =====
  function normalize(v){ // L2正規化でスケール差を除去
    const out = new Float32Array(v.length);
    let n=0; for(let i=0;i<v.length;i++){ n+=v[i]*v[i]; }
    const s = n? 1/Math.sqrt(n):1; for(let i=0;i<v.length;i++) out[i]=v[i]*s; return out;
  }
  ...
  // “今日カウント済みの代表ベクトル” メモリ（ハッシュが揺れてもブロック）
  const personMemory = []; // {vec:Float32Array}  …代表ベクトル
  const MEMORY_SIM_TH = 0.995;   // これ以上似ていれば同一人物扱い（再カウント禁止）

  // 直近観測（追尾用）
  const recent=[];                     // {vec, box, t}
  const trackState=new Map();          // key -> {streak,lastTs,lastBox,countedForever:boolean}
  ...
  function findDup(vec,box,now){
    ...
    for(const r of recent){ if(now-r.t>RECENT_MS) continue; const s=cosSim(vec,r.vec||vec); if(s>=SIM_TH && iou(box,r.box)>=IOU_TH && s>simBest){best=r;simBest=s;} }
    return best;
  }
  ...
  async function hashFaceEmbedding(face){
    ...
    const q = Array.from(norm, v=>Math.round(v/0.02)*0.02);  // 0.02 刻み
  ...
  async function loop(ts){
    ...
    for(const f of faces){
      ...
      // すでにこの“滞在トラック”で確定済みなら何もしない
      if (!st.countedForever && st.streak>=STREAK_N && hash) {

        // 1) personMemory で“似すぎ”判定（ハッシュが揺れても同一人として弾く）
        const samePerson = personMemory.some(p => cosSim(vec, p.vec) >= MEMORY_SIM_TH);

        if (!samePerson) {
          // 2) 日内ユニーク判定（ハッシュ基準）
          if (!uniqSet.has(hash)) {
            uniqSet.add(hash); saveUniqSet(uniqSet);
            minuteCounts[bucket][gkey] += 1;     // ← 日内でこの1回だけ
            // 代表ベクトルとして登録
            personMemory.push({vec});
          }
        }
        // いずれにしても、この滞在中はもう数えない
        st.countedForever = true;
      }
    ...
    renderTable();
    // ★ ここでテキストは出さない（「今日のユニーク合計」を非表示）
    if (logEl) logEl.textContent = '';  // ← 消す
