// === 来客属性カウンター：新規のみカウント（CDNモデル既定・DB永続） ===
(async function boot() {
  // ---------- DOM ----------
  ...
  // モデルの所在：CDN優先・ローカル models/ がある場合は自動で利用
  const CDN_MODELS = 'https://cdn.jsdelivr.net/npm/@vladmandic/human/models';
  let modelBasePath = CDN_MODELS;
  try {
    // 同階層に models/ があればそれを使う（任意）
    await fetch('./models/face-detection.json', {method:'HEAD', cache:'no-store'});
  ...
  // 「一度でもカウントした DB id」は永遠に再カウントしない
  const KEY_COUNTED = 'countedEverIds';
  let countedEver = loadCountedIds();
  function loadCountedIds(){ try{ return new Set(JSON.parse(localStorage.getItem(KEY_COUNTED)||'[]')); }catch{ return new Set(); } }
  function saveCountedIds(set){ localStorage.setItem(KEY_COUNTED, JSON.stringify([...set])); }

  // 今日の特徴量ハッシュ（DB不調時の二重ロック）
  const SITE_SECRET='FIXED_SECRET_12345';
  const kHashSeen = (d)=>`hashSeen:${d}`;
  let hashSeenToday = loadHashSeen(currentDay);

  function loadHashSeen(d){ try{ return new Set(JSON.parse(localStorage.getItem(kHashSeen(d))||'[]')); }catch{ return new Set(); } }
  function saveHashSeen(d,set){ localStorage.setItem(kHashSeen(d), JSON.stringify([...set])); }

  // ---------- IndexedDB（faces-db / vectors） ----------
  const DB_NAME='faces-db', STORE='vectors', DB_VER=3; // 既存より大きく
  function openDB(){
  ...
  async function dbAdd(rec){
    ...
    return new Promise((res,rej)=>{
      ...
      rq.onsuccess=()=>res(rq.result); // 新ID
      rq.onerror  =()=>rej(rq.error);
    });
  }
  ...
  // ---------- トラッキング & 判定 ----------
  const MIN_FACE_SCORE=0.60;        // 以前より少し緩く
  const MIN_AREA_RATIO=0.03;        // 小さすぎる顔を除外
  const STREAK_N=6;                  // 連続フレームで確定（高速化）
  ...
  async function loop(ts){
    ...
    for(const t of [...tracks.values()]){
      ...
      try{
        ...
        if (nearest) {
          ...
          if (loose) {
            ...
            if (!countedEver.has(newId)) {
              countedEver.add(newId); saveCountedIds(countedEver);
        ...
        dbHealthy=false;
        statusEl.textContent='DB不調: カウント停止中（再読み込みで復帰）';
      }
  ...
  function clearAllDailyStorage(){
    const del=[]; for(let i=0;i<localStorage.length;i++){ const k=localStorage.key(i);
      if (k && (k.startsWith('counts:')||k.startsWith('hashSeen:')||k==='countedEverIds')) del.push(k); }
  ...
  async function resetAll(){
    ...
    countedEver=new Set(); saveCountedIds(countedEver);
